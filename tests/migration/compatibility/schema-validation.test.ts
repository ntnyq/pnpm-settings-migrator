...
describe('migratePnpmSettings/schema validation', () => {
  ...
  it.each([
    [
      ...
      'globalShims:\n  node: always',
      'other pnpm version: "globalShims"',
    ],
    [
  ...
  describe.each(['11.26.0', '12.4.0'] as const)(
    'packageConfigs validation for %s',
    targetVersion => {
      it.each([
        [
          'a scalar entry',
        ...
        const original = `sharedWorkspaceLockfile: false\npackages: []\n${yaml}\n`
        await writeWorkspaceYaml(original)

        await expect(
          migratePnpmSettings({ targetVersion, cwd: testDir }),
        ).rejects.toThrow(error)
        await expect(readWorkspaceFile('pnpm-workspace.yaml')).resolves.toBe(
          original,
        )
